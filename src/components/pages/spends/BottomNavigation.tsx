...
function mapLocationToValue(location: Location) {
  if (location.pathname.startsWith(PATHS.SPENDS.INDEX + PATHS.SPENDS.NEW)) return 0
  else if (location.pathname.startsWith(PATHS.SPENDS.INDEX + PATHS.SPENDS.LIST)) return 1
  else
    throw new Error(
...
function BottomNavigation() {
  ...
  const handleChange = (_event: SyntheticEvent, newValue: number) => {
    switch (newValue) {
      case 0:
        navigate(PATHS.SPENDS.INDEX + PATHS.SPENDS.NEW)
        break
      case 1:
        navigate(PATHS.SPENDS.INDEX + PATHS.SPENDS.LIST)
        break
      default:
